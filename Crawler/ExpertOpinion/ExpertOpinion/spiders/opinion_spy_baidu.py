    ...
    def parse(self, response):
        ...
        if(len(db_data) > 0):
            ...
            for article in article_title_a_list:
                article_title_list.append(article.split('"_blank">')[len(article.split('"_blank">'))-1].replace('\t', '').replace('\n', '').replace(' ', '').replace('<em>', '').replace('</em>', '').replace('</a>', ''))
            # 文章来源机构列表
            ...
            for brief in brief_list:
                if(len(brief.split('</p>')) >= 2):
                    article_brief_list.append(brief.split('</p>')[1].split('<span class')[0].replace('\t', '').replace('\n', '').replace(' ', '').replace('<em>', '').replace('</em>', '').replace('</a>', ''))
                else:
            ...
            # 取各个list的最短和最长长度
            min_list_len = min(len(article_url_list), len(article_title_list), len(article_source_list), len(article_time_list), len(article_brief_list), len(snapshot_url_list))
            max_list_len = max(len(article_url_list), len(article_title_list), len(article_source_list), len(article_time_list), len(article_brief_list), len(snapshot_url_list))
            ...
            if((min_list_len > 0) & (min_list_len == max_list_len)):
                ...
                article_title_str = article_title_str.replace('\\', '').replace('\'', '').replace('\"', '').replace('\n', '').replace('\t', '')
                article_brief_str = article_brief_str.replace('\\', '').replace('\'', '').replace('\"', '').replace('\n', '').replace('\t', '')
                
                print('### Database ### ready to store into Database: ' + expert_id + " - " + expert_name)
                sql = cur.execute('insert into edms.opinion_raw(`expert_id`, `expert_name`, `expert_university`, `expert_college`, `source_url`, `article_url`, `article_title`, `article_source`, `article_time`, `article_brief`, `snapshot_url`, `analysis_tag`) values("' + 
                    expert_id + '", "' + expert_name + '", "' + expert_university + '", "' + expert_college + '", "' + source_url + '", "' + article_url_str + '", "' + article_title_str + '", "' + article_source_str + '", "' + article_time_str + '", "' + article_brief_str + '", "' +  snapshot_url_str +  '", "False")')
                conn.commit()
            else:
                # 若无搜索结果，则暂定该专家无观点
                print("no snapshot_url_list")
                print('### Database ### ready to store into Database: ' + expert_id + " - " + expert_name)
                sql = cur.execute('insert into edms.opinion_raw(`expert_id`, `expert_name`, `expert_university`, `expert_college`, `source_url`, `analysis_tag`) values("' + 
                    expert_id + '", "' + expert_name + '", "' + expert_university + '", "' + expert_college + '", "' + source_url +  '", "No")')
                conn.commit()
...
class BaiduNewsSupplementSpider(scrapy.Spider):
    ...
    """
    # 定义爬虫名,在命令行通过"scrapy crawl 爬虫名"来运行爬虫
    name = 'baidu_news_supplement_spider'
    data = []
    df = DataFrame(data=data)
    ...
    def start_requests(self):
        ...
        # 获取opinion_raw表里的专家id数据，为了不爬取已存在的数据
        sql = cur.execute('select `expert_id`, `source_url` from edms.opinion_raw where `article_url` is null and `analysis_tag`!="No"')
        conn.commit()
    ...
    def parse(self, response):
        ...
        print('### Database ### ready to pull data from Database')
        sql = cur.execute('select content from edms.opinion_raw where `expert_id`="' + expert_id + '" and `article_url` is not null')
        conn.commit()
        db_data = cur.fetchall()
        ...
        if(len(db_data) > 0):
            ...
            if((min_list_len > 0) & (min_list_len == max_list_len)):
                ...
                try:
                    ...
                    sql_sentence = 'update edms.opinion_raw set `article_url`="' + article_url_str + '", `article_title`="' + article_title_str + '", `article_source`="' + article_source_str + '", `article_time`="' + article_time_str + '", `article_brief`="' + article_brief_str + '", `snapshot_url`="' + snapshot_url_str + '" where `expert_id`="' + expert_id + '" limit 1'
                    # print(sql_sentence)
                ...
                try:
                    print('### Database ### ready to update Database: ' + expert_id)
                    sql_sentence = 'update edms.opinion_raw set `analysis_tag`="No" where `expert_id`="' + expert_id + '" limit 1'
                    # print(sql_sentence)
                    sql = cur.execute(sql_sentence)
                    # print(sql)
                    if(sql == 1):
...
class BaiduContentSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        sql = cur.execute('select `expert_id`, `article_source`, `article_url` from edms.opinion_raw where `article_url` is not null and `analysis_tag`!="ing"')
        conn.commit()
            ...
            if(len(article_urls_list) > 0):
                for url_index in range(0,len(article_urls_list)):
                    ...
                    if(url_index < len(article_sources_list)):
                        ...
                        yield scrapy.Request(url=article_urls_list[url_index], callback=self.parse, meta={"current_expert_id":current_expert_id, "current_article_source":current_article_source})
            else:
                print('\n该专家无资讯快照')
    ...
    def parse(self, response):
        ...
        if(origin_analysis_tag == 'End'):
            ...
            if(this_content == ''):
                ...
                try:
                    print('### Database ### ready to update Database: ' + expert_id)
                    sql_sentence = 'update edms.opinion_raw set `content`="' + (origin_content + this_content) + '", analysis_tag="ing" where expert_id="' + expert_id + '" limit 1'
                    # print(sql_sentence)
                    sql = cur.execute(sql_sentence)
                    if(sql == 1):
                        print("~~~~~ Update Success ~~~~~")
